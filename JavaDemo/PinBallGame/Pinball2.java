...
public class MainActivity extends Activity {
	...
	// ball
	private int ballX, ballY, ballSize, xSpeed, ySpeed;
	// some flag
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// set ball size
		ballX = randn.nextInt((int) (tableWidht * 0.5));
		ballY = randn.nextInt(randn.nextInt(20));
		//
		ballXX = new int[3];
		ballYY = new int[3];
		xxSpeed = new int[3];
		...
		ballXX[0] = randn.nextInt((int) (tableWidht * 0.5));
		ballYY[0] = randn.nextInt(randn.nextInt(20));
		xxSpeed[0] = (int) (ySpeed * (randn.nextFloat() - 0.5) * 2);
		yySpeed[0] = (int) (tableHeight * 0.04);

		ballXX[1] = (int) (ballXX[0] * (randn.nextFloat() - 0.5) * 2);
		ballYY[1] = randn.nextInt(randn.nextInt(20));
		xxSpeed[1] = (int) (ySpeed * (randn.nextFloat() - 0.5) * 2);
		yySpeed[1] = (int) (tableHeight * 0.03);

		ballXX[2] = (int) (ballXX[0] * (randn.nextFloat() - 0.5) * 2);
		ballYY[2] = randn.nextInt(randn.nextInt(20));
		xxSpeed[2] = (int) (ySpeed * (randn.nextFloat() - 0.5) * 2);
		yySpeed[2] = (int) (tableHeight * 0.02);
				...
				if (beginFlag) {
					...
					timer.schedule(new TimerTask() {
						...
						public void run() {
							for (int i = 0; i < 2; i++) {
								// over left and right
								if (ballXX[i] <= 0 + ballSize
										|| ballXX[i] >= tableWidht - ballSize) {
									xxSpeed[i] = -xxSpeed[i];
								}
								// touch the racket
								if (ballYY[i] <= 0
										|| (ballYY[i] >= racketY - ballSize * 2
												&& ballXX[i] >= racketX && ballXX[i] <= racketX
												+ racketWidht)) {
									yySpeed[i] = -yySpeed[i];

								}
								// touch the bottom
								else if (ballYY[i] >= racketY - ballSize * 2
										&& (ballXX[i] < racketX || ballXX[i] > racketX
												+ racketWidht)) {
									yySpeed[i] = -yySpeed[i];
